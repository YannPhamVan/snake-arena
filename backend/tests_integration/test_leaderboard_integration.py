...
def test_leaderboard_with_multiple_users(client: TestClient, db_session):
    ...
    for token, score, mode in scores:
        ...
        response = client.post("/leaderboard/", json={"score": score, "mode": mode}, headers=headers)
        assert response.status_code == 200
    ...
    # Test mode filtering
    walls_response = client.get("/leaderboard/?mode=walls")
    ...
    passthrough_response = client.get("/leaderboard/?mode=pass-through")
...
def test_score_submission_updates_high_score(client: TestClient, db_session):
    ...
    # Submit first score
    client.post("/leaderboard/", json={"score": 500, "mode": "walls"}, headers=headers)
    ...
    # Submit higher score
    client.post("/leaderboard/", json={"score": 1000, "mode": "walls"}, headers=headers)
    ...
    # Submit lower score
    client.post("/leaderboard/", json={"score": 300, "mode": "walls"}, headers=headers)
...
def test_leaderboard_limit(client: TestClient):
    ...
    for i in range(15):
        ...
        headers = {"Authorization": f"Bearer {token}"}
        client.post("/leaderboard/", json={"score": 100 * i, "mode": "walls"}, headers=headers)
...
def test_leaderboard_requires_auth_for_submission(client: TestClient):
    ...
    response = client.post("/leaderboard/", json={"score": 1000, "mode": "walls"})
    assert response.status_code == 401  # HTTPBearer returns 401 for missing auth
    
    # Try with invalid token
    headers = {"Authorization": "Bearer invalid.token"}
    response = client.post("/leaderboard/", json={"score": 1000, "mode": "walls"}, headers=headers)
