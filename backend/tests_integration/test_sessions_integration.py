...
def test_session_lifecycle(client: TestClient):
    ...
    # Create a session
    create_response = client.post("/api/sessions/", json={"mode": "walls"}, headers=headers)
    ...
    active_response = client.get("/api/sessions/")
    ...
    update_response = client.put(f"/api/sessions/{session_id}", json={"score": 500}, headers=headers)
    ...
    # Update score again
    client.put(f"/api/sessions/{session_id}", json={"score": 1000}, headers=headers)
    get_response = client.get(f"/api/sessions/{session_id}")
    assert get_response.json()["score"] == 1000
    
    # End the session
    end_response = client.delete(f"/api/sessions/{session_id}", headers=headers)
...
def test_multiple_concurrent_sessions(client: TestClient):
    ...
    for i, user in enumerate(users):
        ...
        create_response = client.post("/api/sessions/", json={"mode": mode}, headers=headers)
    ...
    for i, session in enumerate(sessions):
        ...
        score = (i + 1) * 100
        client.put(f"/api/sessions/{session['id']}", json={"score": score}, headers=headers)
    ...
    client.delete(f"/api/sessions/{sessions[0]['id']}", headers=headers)
...
def test_session_operations_require_auth(client: TestClient):
    ...
    response = client.post("/api/sessions/", json={"mode": "walls"})
    ...
    response = client.put(f"/api/sessions/{session_id}", json={"score": 100})
    ...
    assert client.put(f"/api/sessions/{session_id}", json={"score": 100}, headers=headers).status_code == 200
...
def test_nonexistent_session(client: TestClient):
    ...
    response = client.put("/api/sessions/nonexistent-id", json={"score": 100}, headers=headers)
    assert response.status_code == 404
...
def test_session_data_persistence(client: TestClient):
    ...
    # Create session
    create_response = client.post("/api/sessions/", json={"mode": "pass-through"}, headers=headers)
    session_id = create_response.json()["id"]
    
    # Update score multiple times
    for score in [100, 250, 500, 750, 1000]:
        client.put(f"/api/sessions/{session_id}", json={"score": score}, headers=headers)
